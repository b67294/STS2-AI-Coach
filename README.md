...
- 🗺️ **地图侦察**：同步 Spire Codex 结构化资料，在地图页按弱怪、普通怪、精英、Boss 和随机事件池做可视化预警。
...
Python Coach 服务压缩当前局面
    ↓
Markdown / JSON 私域知识上下文 + Spire Codex 本地资料缓存
    ↓
...
地图侦察使用 [Spire Codex](https://github.com/ptrlrd/spire-codex) 作为外部结构化数据源。它提供从游戏文件解析出的怪物、遭遇、事件、章节和图片资源。本项目不会把 Spire Codex 作为 submodule，也不会提交下载后的 JSON 或游戏图片，而是通过同步脚本生成本地缓存：

```text
sts2-coach/data/spire-codex/
```

运行时只读取本地缓存，不会在点击分析时访问外部网站。
...
| 地图侦察 | Spire Codex JSON / 本地图片缓存 | 展示可能怪物、精英、Boss 和随机事件池 |
...
如果需要使用地图页的怪物图、遭遇池和随机事件池，首次运行前同步一次 Spire Codex 本地缓存：
...
该命令会下载结构化 JSON 和图片到 `sts2-coach/data/spire-codex/`。这个目录已被 `.gitignore` 忽略，不会上传到 GitHub。

以下情况建议重新同步：

- 第一次 clone 项目后
- 换电脑或删除了本地缓存
- 游戏或 Spire Codex 数据更新后
